...
class ImageTransforms(object):
    ...
    def __call__(self, img):
        ...
        ######## 在这里加滤波&噪音，注意保持图片尺寸大小不变 ########
        #lr_img = lr_img.filter(ImageFilter.MedianFilter(size=3))    ###### 新加的（中值滤波） zhangcan ###########
        lr_img = lr_img.filter(ImageFilter.GaussianBlur(radius=1))    ###### 新加的（高斯模糊） zhangcan ###########
